    ...
    <div className="flex bg-gray-50">
      ...
      <main className="flex-1 pb-20 lg:pb-0">
        <div className="p-4 md:p-6 max-w-7xl mx-auto">
          <Link href="/stores" className="inline-flex items-center text-gray-500 hover:text-cyan-600 text-sm font-medium mb-4 transition-colors">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex items-center gap-4">
              <img src={store.logo} alt={store.name} className="w-20 h-20 rounded-xl border border-gray-100 shadow-sm object-cover" />
              <div>
                <h1 className="text-2xl font-bold text-gray-900">{store.name}</h1>
                <p className="text-gray-500 font-medium mb-2">{store.type}</p>
                <div className="flex items-center gap-2">
                  <Badge color={store.status === 'ACTIVE' ? 'primary' : 'gray'} className="px-2.5 py-0.5 rounded-full text-xs font-bold shadow-none">
                    {store.status}
                  </Badge>
                  <span className={`text-sm font-semibold ${store.trendPct > 0 ? 'text-green-600' : 'text-red-600'}`}>
                 ...
                 <DialogContent className="sm:max-w-[425px]">
                   ...
                   <div className="grid gap-4 py-4">
                     <div className="flex flex-col gap-2 relative">
                        <label className="text-sm font-semibold">Store Logo Image</label>
                        <div className="flex items-center gap-4">
                          <img src={formData.logo} alt="Preview" className="w-16 h-16 rounded-xl object-cover border" />
                          <Input 
                            value={formData.logo} 
                     ...
                     <div className="flex flex-col gap-2">
                        <label className="text-sm font-semibold">Store Category</label>
                        <Input 
                          value={formData.type} 
          ...
          {/* Chart Section */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6 mb-6 transition-all duration-300 hover:shadow-md">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={chartData}>
                ...
                <YAxis stroke="#9ca3af" axisLine={false} tickLine={false} tickFormatter={(val) => `Rs. ${val}`} />
                <Tooltip
